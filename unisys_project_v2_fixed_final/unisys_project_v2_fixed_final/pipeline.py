...
def run_pipeline(csv_path, label_col, target_size):
    ...
    for step, row in enumerate(synth_pool):
        ...
        if len(batch_rows) >= BATCH_SIZE:
            ...
            # Fix 6: accept arm gets the delta-F1 reward (it drove the outcome);
            #         reject arm always gets 0.0 — it contributed nothing to F1 change.
            #         This prevents the arms from learning the same signal and diverging properly.
            for a, is_warm_step in batch_actions:
                if not is_warm_step:
                    if a == 1:
                        bandit.update(1, reward)   # accept arm: reward tied to F1 delta
                    else:
                        bandit.update(0, 0.0)      # reject arm: neutral — it had no effect
