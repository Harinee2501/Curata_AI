...
class ThompsonBandit:
    ...
    def select(self, context: np.ndarray) -> int:
        ...
        # Boost accept arm by sample quality
        context_quality = float(np.mean(np.clip(context, 0.0, 1.0)))
        samples[1] *= (1.0 + context_quality)
    ...
    def update(self, arm: int, reward: float):
        ...
        Positive ΔF1 = success (alpha++), non-positive = failure (beta++).
        """
