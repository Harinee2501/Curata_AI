...
# ── Override thresholds ───────────────────────────────────────────────
# If the unfiltered pool has at least this many rows, force a FILTER
# rather than letting the policy spin on EVALUATE.
_FORCE_FILTER_UNFILTERED_MIN = 10

# After this many curated rows accumulate without a retrain, force one.
_FORCE_RETRAIN_CURATED_MIN = 20

# If the policy picks EVALUATE more than this many times in a row
# while there are still rows to process or curated rows to retrain on,
# override with a more useful action.
...
# ─────────────────────────────────────────────────────────────────────
# EPISODE STATE — obs layout mirrors DataCurationEnv._get_obs exactly
# ─────────────────────────────────────────────────────────────────────
...
class _EpisodeState:
    ...
    def __init__(
        ...
        self.prev_f1               = float(baseline_f1)   # updated each step
        ...
        # ── Override tracking ─────────────────────────────────────
        self.consecutive_evaluate      = 0   # how many EVALUATE in a row
        self.curated_since_last_retrain = 0  # rows curated since last retrain
    ...
    def observe_f1(self, new_f1: float) -> None:
        nf = float(new_f1)
        self.prev_f1    = self.current_f1     # snapshot BEFORE update
        self.current_f1 = nf
    ...
    def to_obs(self) -> np.ndarray:
        """
        Build the 29-float observation vector.
        Order and normalisation MUST match DataCurationEnv._get_obs() exactly.
        """
        ...
        # [25] stop_attractiveness  = consecutive_no_improve / 5
        stop_attractiveness = float(np.clip(self.consecutive_no_improve / 5.0, 0, 1))

        # [26] budget_urgency = budget_used / total_budget
        ...
        obs = np.array(
            [
                ...
                *last_action_onehot,
                ...
                # [26] budget urgency = budget_used / total_budget
                budget_urgency,

                # [27] f1_vs_best shifted to [0,1]
...
def _load_vec_normalize_for_inference(vecnormalize_path: str):
    """
    Restore SB3 VecNormalize stats for inference-time obs normalisation.
    """
...
def _override_action(
    ...
    """
    Inspect the policy's chosen action and override it when the agent
    is clearly stuck (e.g. EVALUATE loop while rows are waiting).

    Returns (final_action, override_reason_or_empty_string).
    """
    needs_more_curated = curated_len < state.target_size
    budget_remaining   = state.total_budget - state.budget_used

    # ── 1. Unfiltered pool is large enough → force FILTER ─────────────
    if (
    ...
    # ── 2. Enough new curated rows since last retrain → force RETRAIN ──
    if (
        state.curated_since_last_retrain >= _FORCE_RETRAIN_CURATED_MIN
        and curated_len > 0
        and policy_action not in (
    ...
    # ── 3. Policy is stuck in EVALUATE loop ───────────────────────────
    if (
        ...
        # Prefer FILTER if there's anything to filter, else GEN
        if unfiltered_len > 0:
            return (
                DataCurationEnv.ACTION_FILTER,
                f"[OVERRIDE→FILTER] broke EVALUATE loop ({state.consecutive_evaluate} in a row)",
            )
        # Only generate if we have budget
        ...
        if budget_remaining >= batch:
            return (
                action,
                f"[OVERRIDE→GEN] broke EVALUATE loop, pool empty",
            )

    # ── 4. Policy wants STOP but target not reached and budget remains ─
    if (
        ...
        if unfiltered_len >= _FORCE_FILTER_UNFILTERED_MIN:
            return (
                DataCurationEnv.ACTION_FILTER,
                f"[OVERRIDE→FILTER] blocked premature STOP, pool has rows",
            )
        batch = 50 if budget_remaining >= 50 else 10
        action = (
            DataCurationEnv.ACTION_GEN_LARGE
        ...
        return (
            action,
            f"[OVERRIDE→GEN] blocked premature STOP, need more rows",
        )
...
class RLAgent:
    ...
    max_steps : int
        Hard episode cap — matches DataCurationEnv.MAX_STEPS at training time.
    vecnormalize_path : str | None
        Path to vecnormalize_final.pkl from the same training run.
        Required when the policy was trained with VecNormalize (it was).
    """
    ...
    def __init__(
        ...
        if vecnormalize_path:
            ...
            if p.is_file():
                try:
                    self._vec_normalize = _load_vec_normalize_for_inference(str(p.resolve()))
        ...
        logger.info(f"[RLAgent] Policy ready ✓  (obs_dim={policy_dim})")

    def _normalise_obs(self, obs: np.ndarray) -> np.ndarray:
        """Apply VecNormalize stats if loaded; otherwise return raw obs."""
    ...
    def run_episode(
        ...
        # ── Validate obs dim before starting ──────────────────────────
        probe = state.to_obs()
        ...
        for step in range(self.max_steps):
            ...
            if override_reason:
                logger.info(
                    f"  {_C['yellow']}{override_reason}{_C['reset']}"
                )

            # Update loop counter BEFORE dispatch (mirrors env.step)
            ...
            # Track consecutive EVALUATE for override logic
            ...
            if action == self.ACTION_GEN_SMALL:
                new_rows = self._do_generate(
                    df, label_col, label_names, content_cols,
                    batch_size=10, state=state, generate_fn=generate_fn,
                )
                ...
                new_rows = self._do_generate(
                    df, label_col, label_names, content_cols,
                    batch_size=50, state=state, generate_fn=generate_fn,
                )
                unfiltered.extend(new_rows)
                extra_note = f"+{len(new_rows)} rows → unfiltered pool"
                ...
                if unfiltered:
                    ...
                    extra_note = f"accepted {len(accepted)}/{n_pool} (rate={acc_rate:.0%})"
                else:
                ...
                if curated_X:
                    ...
                    state.observe_f1(new_f1)       # updates prev_f1 + best_f1_so_far
                    state.n_retrains += 1
                    state.curated_since_last_retrain = 0   # reset counter
                    state.minority_class_f1 = float(np.clip(
                        state.minority_class_f1 + (new_f1 - prev_f1_this_step) * 1.2,
                        0.0, 1.0,
                    ))
            ...
            # ── Per-step state updates (mirror env.step) ──────────────
            # prev_f1 is already updated inside observe_f1() for retrain/eval.
            # For non-f1-changing actions, set it here so obs[1] is correct.
    ...
    def _do_generate(
        self, df, label_col, label_names, content_cols,
        batch_size, state, generate_fn,
    ) -> list:
...
def _budget_cost(action: int) -> int:
    # Matches DataCurationEnv:
    #   GEN_SMALL=10, GEN_LARGE=50, FILTER=0, RETRAIN=0, EVALUATE=2, STOP=0
